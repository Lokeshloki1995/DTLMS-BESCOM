  ...
  <%--  $('#<%=cmdSave.ClientID %>').prop('disabled', false);--%>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            $("#ContentPlaceHolder1_grdMaterialMast_txtMqty_0").on("input", function () {
                debugger;
                // Get the entered value
                var enteredValue = $(this).val();

                // Define a regular expression for valid input (allowing only numbers and one dot)
                var regex = /^\d+(\.\d{0,2})?$/;

                // Test the entered value against the regular expression
                if (!regex.test(enteredValue)) {
                    $(this).val(enteredValue.replace(/[^\d.]/g, ''));
                }
            });
        });
        ...
        //        // Remove leading non-digit characters (except dots)
        //        var sanitizedValue = enteredValue.replace(/^[^\d.]+/, '');
        ...
        //        // Test the sanitized value against the regular expression
        //        if (!regex.test(sanitizedValue)) {
        //            // If it doesn't match, replace the input value with the sanitized version
        //            $(this).val(sanitizedValue.replace(/[^\d.]/g, ''));
        //        }
        //    });
        //});
